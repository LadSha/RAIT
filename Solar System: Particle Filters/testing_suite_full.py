...
GLOBAL_PARAMETERS = [
    ...
    # Case 1 has no noise to make things easy for you!
    {
        ...
        "planets": [
            ((-0.9122000568392201, 0.307965701950435), 743.109985022544),
    ...
    },
    {
        ...
        "planets": [
            ((0.9458667144043527, 0.05816442524818104), 327.84830063909425),
    ...
    },
    {
        ...
        "planets": [
            ((-0.8822853436169761, 0.35956874874350164), 202.629302718903),
    ...
    },
    {
        ...
        "planets": [
            ((0.6348717055275535, -0.6923360388055648), 593.3821544996055),
    ...
    },
    {
        ...
        "planets": [
            ((-0.6159704854212522, 0.7572261480543556), 745.260269507586),
...
class PlanetSimulator(object):
    ...
    def simulate_without_sos(self, estimate_next_pos, params):
        ...
        solar_system = SolarSystem(
            mass_sun=MASS_SUN * params["sun_mass"],
        ...
        target = SolarSystem.init_body_in_orbit_at_x_and_y(
            mass_sun=solar_system.sun.mass, r=target_r, mass_body=target_mass
        )
        ...
        try:
            while steps < params["max_steps"]:
                ...
                def compute_gravity_magnitude_at_x_y(planets, x, y):
                    ...
                    for body in planets:
                        ...
                        c = (
                            G
                            * body.mass
                            / (direction[0] ** 2 + direction[1] ** 2) ** (3.0 / 2)
                        )
                ...
                gravimeter_measurement = sense(
                    target.r[0], target.r[1], params["g_measurement_noise"]
                )
                target2 = solar_system.move_body(copy.deepcopy(target))
                radius = math.sqrt(target.r[0] ** 2 + target.r[1] ** 2)
                beta_turning_angle = (
                    math.atan2(target2.v[1], target2.v[0])
                    - math.atan2(target.v[1], target.v[0])
                    + 2 * math.pi
                ) % (2 * math.pi)
                ...
                if len(result) == 3:
                    ...
                    if not isinstance(optional_points_to_plot, list):
                        raise TypeError(
                            f"Expected returned optional_points_to_plot to be a list "
                    ...
                    if len(optional_points_to_plot) > 0:
                        if not isinstance(optional_points_to_plot[0], tuple):
                            raise TypeError(
                                f"Expected the element in optional_points_to_plot to be a tuple "
                                f"but it was actually a {type(optional_points_to_plot[0])}"
                            )
                        if (
                            len(optional_points_to_plot[0]) < 2
                            or len(optional_points_to_plot[0]) > 3
                        ):
                            raise TypeError(
                                f"Expected the element in optional_points_to_plot to have 2-3 elements, "
                ...
                if not isinstance(xy_estimate, tuple):
                    raise TypeError(
                        f"Expected the returned xy_estimate to be a tuple "
                        f"but it was actually a {type(xy_estimate)}"
                    )
                if len(xy_estimate) != 2:
                    raise TypeError(
                        f"Expected the returned xy_estimate to have 2 elements, such as (x,y), "
                ...
                if PLOT_PARTICLES:
                    ...
                    if target_turtle is None:
                        ...
                        target_turtle.shapesize(0.25, 0.25)
                    ...
                    satellite_trail.append(
                        (target_pos[0] / AU, target_pos[1] / AU)
                    )  # Add coordinates to the trail list
                    ...
                    if estimate_turtle is None:
                        ...
                        estimate_turtle.shapesize(0.2, 0.2)
                        estimate_turtle.fill = False
                        estimate_turtle.color("cyan")
                        estimate_turtle.penup()
                    estimate_turtle.setposition(
                        xy_estimate[0] / AU, xy_estimate[1] / AU
                    )
                    ...
                    if sun_turtle is None:
                        ...
                        sun_turtle.shapesize(0.75, 0.75)
                    ...
                    if planet_turtle_list is None:
                        ...
                        planet_trail = [
                            [] for i in range(solar_system.get_num_planets())
                        ]

                        planet_turtle_list = [
                            turtle.Turtle()
                            for _ in range(solar_system.get_num_planets())
                        ]

                        for i, planet_turtle in enumerate(planet_turtle_list or []):
                            size = (
                                0.1
                                + solar_system.planets[i].mass / MASS_EARTH / 1000 * 0.4
                            )
                            ...
                            planet_trail[i].append(
                                (x, y)
                            )  # Add initial coordinates to the trail list
                        # Create trail turtle list and set base turtle settings
                        planet_trail_list = [
                            turtle.Turtle()
                            for _ in range(solar_system.get_num_planets())
                        ]
                        for ti, planet_trail_turtle in enumerate(
                            planet_trail_list or []
                        ):
                        ...
                        if len(planet_trail[0]) >= TRAIL_LENGTH:
                            for ti, planet_trail_turtle in enumerate(
                                ...
                                xt_yt = planet_trail[ti][
                                    len(planet_trail[0]) - TRAIL_LENGTH
                                ]
                ...
                if VERBOSE is True:
                    print(
                        "\nStep: {} Actual ({})  Predicted: ({})\n  Difference = {}\n  Gravity Magnitude={}".format(
                            ...
                            xy_estimate,
                            separation,
                            gravimeter_measurement,
                        )
                    )
                    if (
                        optional_points_to_plot is not None
                        and len(optional_points_to_plot) > 0
                    ):
                        particle_dist = []
                        ...
                        print(
                            "{} Particles, Min dist: {}, Avg dist: {}, Max Dist: {}".format(
                                len(optional_points_to_plot), pMin, pAvg, pMax
                            )
                        )
        ...
        try:
            while steps < params["max_steps"]:
                ...
                def get_percent_illumination(a, b, c):
                    ...
                    phase_angle = math.atan2(c[1] - b[1], c[0] - b[0]) - math.atan2(
                        a[1] - b[1], a[0] - b[0]
                    )
                    phase_angle = (
                        phase_angle + 2 * math.pi if phase_angle < 0 else phase_angle
                    )
                ...
                def percent_illuminations_sense_func(x: float, y: float):
                    return [
                        get_percent_illumination(solar_system.sun.r, planet.r, (x, y))
                ...
                def sense(x: float, y: float):
                    return [
                        random.gauss(
                            get_percent_illumination(
                                solar_system.sun.r, planet.r, (x, y)
                            ),
                ...
                if not isinstance(bearing, float):
                    raise TypeError(
                        f"Expected the returned bearing to be a float "
                ...
                if steps % sos_trans_steps == 0:
                    ...
                    t0 = (home_planet_vect[0] - target_x) * math.cos(bearing) + (
                        home_planet_vect[1] - target_y
                    ) * math.sin(bearing)

                    closest_point = (
                        target_x + t0 * math.cos(bearing),
                        target_y + t0 * math.sin(bearing),
                    )
                    separation = self.distance(home_planet_loc, closest_point)

                    point1 = (target_x / AU, target_y / AU)
                    point2 = (
                        target_x / AU + WINDOW_SIZE * math.cos(bearing),
                        target_y / AU + WINDOW_SIZE * math.sin(bearing),
                    )

                    if separation < tolerance:
                        point2 = (
                            (target_x + t0 * math.cos(bearing)) / AU,
                            (target_y + t0 * math.sin(bearing)) / AU,
                        )
                        hit = True
                        hits += 1
                    else:
...
class CaseRunner(unittest.TestCase):
    ...
    def run_with_params(self, k, test_params, test_method, student_method):
        ...
        if DEBUGGING_SINGLE_PROCESS:
            ...
            test_process = mproc.Process(
                target=test_method, args=(student_method, test_params)
            )
            ...
            if test_process.is_alive():
                ...
                error_message = (
                    "Test aborted due to CPU timeout. "
                    + "Test was expected to finish in fewer than {} second(s).".format(
                        TIME_LIMIT
                    )
                )
                print(error_message)
...
if file_checker:
    ...
    if len(modified_files) == 0:
        print(
            "You are running against the same framework as the Gradescope autograder."
        )
    else:
        print(
            "Warning. The following files have been modified and the results may not be accurate:"
        )
...
if __name__ == "__main__":
    if stack_trace:
        ...
        if student_id:
            ...
            suites = [
                unittest.TestSuite(unittest.TestLoader().loadTestsFromTestCase(case))
                for case in cases
            ]
            ...
            try:
                for i, suite in zip(list(range(1, 1 + len(suites))), suites):
                    ...
                    print(
                        "Successes: {}\nFailures: {}\n".format(
                            num_passes, num_errors + num_fails
                        )
                    )
