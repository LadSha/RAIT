...
FAILURE_UNIMPLEMENTED = 'please_implement_estimation'
...
def run_estimation(field,
    ...
    num_req_timesteps_estimates_gt_90pct = 5
...
def run_defense(field,
    ...
    field.laser_or_ground_hit(INIT_IMMUNITY_TIME, 0,
    ...
    for timesteps in range(0, nsteps):
        ...
        # the simulation time has exceeded INIT_IMMUNITY_TIME
        if t > INIT_IMMUNITY_TIME:
            # if we are past the immunity period of INIT_IMMUNITY_TIME, deduct 
            # one health point for each meteorite that hits the ground
        ...
        laser_angle_change_rad, should_laser_fire_tp1 = \
            turret.get_laser_action(laser_angle_rad)

        laser_is_on = False
        if not type(laser_angle_change_rad) == bool and \
                isinstance(laser_angle_change_rad, (float, int)):
            # laser_angle_change_rad is the change in the laser's aim angle for
            # t+1 (in radians). Add it to the laser's previous angle (from
            # time t)
            laser_angle_change = float(laser_angle_change_rad)
            ...
            print("The first return value from the Turret class's get_laser_action should be a float or an int!")

        if type(should_laser_fire_tp1) == bool:
            if should_laser_fire_tp1:
                # if the laser should fire at time t+1, turn it on
                laser_is_on = True
        else:
            print("The second return value from the Turret class's get_laser_action should be a bool!")
