...
# Note: the prefilled answers from the video have already been entered for you
# and are not graded
#
Q1_Omega = [
    [4, -1, 0, -2, 0],
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    """
    :param data: (list) returned from make_data function
    :param N: (int) the total number of poses (synonymous with number of time steps) that the robot exists in throughout the simulation
    ...
    # Ref: code is taken from lecture material https://gatech.instructure.com/courses/325394/pages/2-online-slam-answer?module_item_id=3202984
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            for b in range(2):
                ...
                Xi.value[b][0] += -measurement[i][b + 1] / measurement_noise
                Xi.value[m + b][0] += measurement[i][b + 1] / measurement_noise

        # add the extra column and row here
        outliner = [0, 1] + [i for i in range(4, dim + 2)]
        Omega = Omega.expand(dim + 2, dim + 2, outliner, outliner)
        Xi = Xi.expand(dim + 2, 1, outliner, [0])
...
class robot(object):
    ...
    def sense(self):
        ...
        for i in range(self.num_landmarks):
            ...
            if (
                self.measurement_range < 0.0
                or abs(dx) + abs(dy) <= self.measurement_range
            ):
                Z.append([i, dx, dy])
        return Z
...
testdata1 = [
    [
        [
            [1, 21.796713239511305, 25.32184135169971],
...
testdata2 = [
    [
        [
            [0, 12.637647070797396, 17.45189715769647],
...
def format_array_output(ans_arrays):
    mu_str = checkutil.format_2d_arr(ans_arrays[0].value)
    omega_str = checkutil.format_2d_arr(ans_arrays[1].value)
